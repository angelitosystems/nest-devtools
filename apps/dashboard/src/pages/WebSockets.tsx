      ...
      <section className="bg-surface-850 border border-surface-600 rounded-lg overflow-hidden">
        ...
        {messages.length === 0 ? <div className="p-8 text-center text-sm text-slate-500">No WebSocket messages captured yet.</div> : (
          <ul className="divide-y divide-surface-700 font-mono text-xs">
            {messages.slice(0, 500).map((item, index) => <li key={`${item.timestamp}-${index}`} className="flex items-center gap-3 px-4 py-2 hover:bg-surface-800"><span className="text-slate-600">{formatTime(item.timestamp)}</span><span className={cn('flex items-center gap-1 w-24', item.direction === 'sent' ? 'text-sky-300' : 'text-emerald-300')}>{item.direction === 'sent' ? <ArrowUpFromLine className="h-3.5 w-3.5" /> : <ArrowDownToLine className="h-3.5 w-3.5" />}{item.direction}</span><span className="text-accent-300">{item.gateway}</span><span className="text-slate-200 truncate">{item.event}</span><span className="ml-auto text-slate-500">{item.payloadSize} B</span>{item.error && <span className="text-rose-400 truncate max-w-48">{item.error}</span>}</li>)}
          </ul>
        )}
      </section>
    </div>
