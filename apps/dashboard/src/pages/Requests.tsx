...
export default function RequestsPage({ store }: { store: DevToolsState }) {
  ...
  const selected =
    filtered.find((request) => request.requestId === selectedId) ?? filtered[0];
        ...
        <div className="bg-surface-850 border border-surface-600 rounded-lg overflow-auto flex-1">
          {filtered.length === 0 ? (
            <div className="p-10 text-center text-sm text-slate-500">
              No requests yet — they appear here in real time.
            </div>
          ) : (
            <table className="w-full font-mono text-xs">
              ...
              <tbody className="divide-y divide-surface-700">
                {filtered.slice(0, 200).map((request) => (
                  ...
                  >
                    <td className="px-4 py-1.5 text-accent-400">
                      {request.method}
                    </td>
                    <td className="px-2 py-1.5 text-slate-200 truncate max-w-[280px]">
                      {request.url}
                    </td>
                    <td
                      className={cn(
    ...
    <aside className="w-96 shrink-0 bg-surface-850 border border-surface-600 rounded-lg overflow-auto">
      <header className="px-4 py-3 border-b border-surface-700">
        <div className="font-mono text-sm">
          <span className="text-accent-400">{request.method}</span>{" "}
          <span className="text-slate-200">{request.url}</span>
        </div>
...
function groupByLayer(
  timeline: RequestCompletedPayload["timeline"],
): Array<[string, typeof timeline]> {
