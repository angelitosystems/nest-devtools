...
export default function PerformancePage({ store }: { store: DevToolsState }) {
  ...
  if (!latest) {
    return <EmptyState title="Waiting for performance data" hint="Connects automatically once an app links the SDK." icon={<Activity className="h-6 w-6" />} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        ...
        />
        <GaugeCard icon={<Activity className="h-4 w-4 text-accent-400" />} label="Req/s" value={String(latest.requestsPerSecond)} percent={Math.min(100, latest.requestsPerSecond)} />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <ChartCard title="CPU %" data={points.map((p) => p.cpuPercent)} color="#e63462" />
        <ChartCard title="Heap used" data={points.map((p) => p.heapUsedBytes / 1024 / 1024)} color="#4d94f0" unit="MB" />
        <ChartCard title="Latency p95 (ms)" data={points.map((p) => p.p95LatencyMs)} color="#e5a52c" />
        <ChartCard title="Event loop lag (ms)" data={points.map((p) => p.eventLoopLagMs)} color="#3fb970" />
      </div>

      <div className="rounded-lg border border-surface-600 bg-surface-850 p-4">
        <h3 className="mb-3 text-sm font-medium">Latency percentiles</h3>
...
function GaugeCard({ icon, label, value, percent }: { icon: React.ReactNode; label: string; value: string; percent: number }) {
  return (
    <div className="rounded-lg border border-surface-600 bg-surface-850 p-4">
      ...
      <div className="mt-2 h-1.5 rounded bg-surface-700">
        <div
  ...
  return (
    <div className="rounded-lg border border-surface-600 bg-surface-850 p-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="font-mono text-xs text-slate-500">{data.length > 0 ? `${round(data[data.length - 1])}${unit ? ` ${unit}` : ''}` : '—'}</span>
      </div>
